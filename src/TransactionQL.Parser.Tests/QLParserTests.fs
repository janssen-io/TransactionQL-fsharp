...
let ``Expressions: nested`` () =
    test
        ...
        (Add(
            ...
            Subtract(
                Variable "total",
                Multiply(Divide(ExprNum 1.0, ExprNum 2.0), Variable "remainder")
            )
        ))
...
let ``Accounts: words separated by colons`` () =
    test
        QLParser.qaccount
        "Expenses:Recreation:Hobby"
        (AccountLiteral [ "Expenses" ; "Recreation" ; "Hobby" ])
...
let ``Posting: two transactions`` () =
    ...
    test
        ...
        (Posting(
            ...
            [
                trx(
                    AccountLiteral [ "Expenses" ; "Rent" ],
                    Some <| AmountExpression(Commodity "EUR", Variable "total")
                )
            ...
            Posting(
                ...
                [
                    trx(
                        ...
                        Some
                        <| AmountExpression(Commodity "EUR", Divide(Variable "total", ExprNum 2.0))
                    )
                    trx(
