...
let ``Expressions: nested`` () =
    ...
    let actual =
        eval'(
            ...
            Divide(Add(Variable "total", Variable "remainder"), ExprNum 2.0)
        )
...
let ``Accounts: variable`` () =
    ...
    let env' = {
        env with
            EnvVars = Map.ofList [ ("account:default", "Default:Checking") ]
    }
...
let ``Payee: variable`` () =
    ...
    let env' = {
        env with
            Row = Map.ofList [ ("Name", "American Express") ]
    }
...
let ``Posting: multiple lines`` () =
    ...
    let transactions = [
        trx(
            AccountLiteral [ "Expenses" ; "Food" ],
            Some <| AmountExpression(Commodity "€", ExprNum 20.00)
        )
